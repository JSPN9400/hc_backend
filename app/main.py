...
def run_seed():
    ...
    try:
        ...
        from app.models.models import (
            Tenant, User, Site, Worker, PlanEnum, RoleEnum, WorkerTypeEnum
        )
        ...
        if not t:
            t = Tenant(id=gen_id(), name="Vishwanath Construction",
        ...
        USERS = [
            ...
            ("HR","hr","hr123",RoleEnum.hr,False,True,True,False,True,True,False,True),
        ]
        for name,uname,pw,role,s,w,a,e,sal,r,u,ed in USERS:
            if not db.query(User).filter(User.tenant_id==TENANT_ID,User.username==uname).first():
                db.add(User(id=gen_id(),tenant_id=TENANT_ID,name=name,username=uname,
        ...
        db.commit(); db.close()
        return {"status":"✅ Seed complete!","tenant":"Vishwanath Construction",
            "logins":{"super_admin":"jaisankar / jai@2024","admin":"admin / admin123","accounts":"anish / anish123"}}
    except Exception as e:
